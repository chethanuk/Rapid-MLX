...
def _create(desktop_manifest, app_dir, dmg, **overrides):
    kwargs = dict(
        app_dir=app_dir, dmg_path=dmg, source_sha=_SHA, version="0.13.0-rc2",
        app_tag="rapid-mac-v0.13.0-rc2", signed=True, delta_compared=True,
    )
...
def test_create_then_verify_round_trips(desktop_manifest, tmp_path):
    ...
    assert desktop_manifest.verify_manifest(
...
def test_accepts_stable_version_and_unspecified_signed(desktop_manifest, tmp_path):
    ...
    dmg = _dmg(tmp_path, app_dir)
    manifest = _create(
        desktop_manifest, app_dir, dmg, version="0.13.1", app_tag="rapid-mac-v0.13.1",
        signed=None,
    )
...
def test_verify_rejects_changed_dmg(desktop_manifest, tmp_path):
    ...
    with pytest.raises(ValueError, match="SHA-256 mismatch|size mismatch"):
        desktop_manifest.verify_manifest(app_dir=app_dir, dmg_path=dmg, manifest_path=output)
...
def test_create_rejects_invalid_version(desktop_manifest, tmp_path):
    ...
    with pytest.raises(ValueError, match="invalid release version"):
        _create(desktop_manifest, app_dir, dmg, version="not-a-version",
...
def test_verify_rejects_embedded_version_drift(desktop_manifest, tmp_path):
    ...
    (app_dir / "build" / "Rapid-MLX Desktop.app" / "Contents" / "Info.plist").write_bytes(
        plistlib.dumps({
            "CFBundleIdentifier": "com.rapidmlx.rapid",
            "CFBundleShortVersionString": "0.13.0-rc2",
            "CFBundleVersion": "165",
...
def test_signed_manifest_claims_signing_and_notary_gates(desktop_manifest, tmp_path):
    ...
    app_dir = _app(tmp_path)
    dmg = _dmg(tmp_path, app_dir)
    manifest = _create(desktop_manifest, app_dir, dmg)  # signed=True, delta_compared=True
    gates = manifest["validation_gate"].split("|")
    assert manifest["signed"] is True
    for claimed in ("signed-build", "app-notarize", "dmg-build", "dmg-size-delta",
...
def test_unsigned_manifest_never_claims_signing_or_notary_gates(desktop_manifest, tmp_path):
...
def test_manifest_does_not_claim_dmg_size_delta_when_skipped(desktop_manifest, tmp_path):
...
def test_verify_rejects_self_claimed_signed_gates_for_unsigned_build(desktop_manifest, tmp_path):
    ...
    app_dir = _app(tmp_path)
    dmg = _dmg(tmp_path, app_dir)
    manifest = _create(desktop_manifest, app_dir, dmg, signed=False, delta_compared=True)
    ...
    desktop_manifest.write_manifest(manifest, output)
    with pytest.raises(ValueError, match="validation gate does not match what actually ran"):
        desktop_manifest.verify_manifest(app_dir=app_dir, dmg_path=dmg, manifest_path=output)
...
def test_verify_rejects_claimed_delta_when_not_compared(desktop_manifest, tmp_path):
    ...
    app_dir = _app(tmp_path)
    dmg = _dmg(tmp_path, app_dir)
    manifest = _create(desktop_manifest, app_dir, dmg, signed=True, delta_compared=False)
    manifest["validation_gate"] = (
        "signed-build|bundle-size|app-notarize|dmg-build|dmg-size-delta|"
        "validate-dmg|dmg-notarize|final-validate-dmg"
    )
