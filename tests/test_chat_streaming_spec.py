...
def test_public_stream_id_is_the_scheduler_request_id():
    """The first public SSE id must be the id admitted by the engine.

    Clients can only cancel with the ``chatcmpl-*`` id exposed on the wire.
    A separately generated scheduler UUID makes that documented endpoint
    return 404 while generation continues.
    """
    ...
    events, saw_done = _parse_sse_events(response.text)
    assert saw_done
    public_id = events[0]["id"]
    assert public_id.startswith("chatcmpl-")
