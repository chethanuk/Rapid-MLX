...
- **Default review cap: 20 rounds.** A task-specific cap set by the owner or Atlas always wins and may be lower; never extend a cap silently. Count every reviewer request as a round. Most PRs still converge well before the cap — the cap is a ceiling, not a target.
- **Reviewers never expand the PR's scope.** This binds the automated adversarial review and human reviewers alike. The PR contract (`## Scope` / `## Non-goals` in the description) bounds what a round may demand: a finding outside the declared Scope/Non-goals becomes a follow-up issue, not an additional round. Pre-existing defects, optional hardening, adjacent redesign, and unrelated improvements are follow-ups by default, never a reason to keep iterating on this PR.
- **The PR owner dispositions every finding before requesting another round** — fix it, open a follow-up issue, or reject it with a stated reason. The P0/P1 dismissal evidence bar above applies to in-scope findings; an out-of-scope finding is dispositioned by linking the follow-up issue.
- **Round 11 and later are convergence rounds.** Post a short scope ledger in the PR thread (which in-scope finding is still open, what was dispositioned as follow-up, the remaining bound) and limit the round to (a) the unresolved in-scope finding, (b) direct regressions from its fix, and (c) material risk caused by the contracted diff. Nothing else is in bounds. If rounds keep surfacing *new* in-scope P0s past this point, the PR scope is too large — split it.
- **If a required fix genuinely crosses the PR boundary**, the owner asks Atlas whether to expand the PR intentionally (updating `## Scope` / `## Non-goals` in the description) or to open a separate follow-up. Scope expansion is a deliberate, recorded decision — never a side effect of a review round.
