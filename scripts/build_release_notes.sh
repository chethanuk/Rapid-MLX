...
# Companion file:
#   strip_release_note_comments.py must remain beside this script.
# Optional env:
...
NORMALIZER="$SCRIPT_DIR/strip_release_note_comments.py"
...
case "$REASON" in
  *$'\r'*|*$'\n'*)
    echo "REASON must be a single line" >&2
    exit 1
    ;;
esac
...
if git cat-file -e "$RELEASE_SHA:$HIGHLIGHTS_PATH" 2>/dev/null; then
  HIGHLIGHTS=$(
    git show "$RELEASE_SHA:$HIGHLIGHTS_PATH" |
      # Keep preflight and publisher on one executable normalization SSOT.
      # The iterative parser preserves code examples, removes every drafting
      # comment in byte order, and drops an unmatched comment tail fail-closed.
      python3 "$NORMALIZER" |
